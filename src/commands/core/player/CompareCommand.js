...
class CompareCommand extends Command {
  ...
  async buildMessage (client, args) {
    ...
    const secondId = PlayerManager.locatePlayer(args[1])
    const firstPlayer = PlayerManager.players.get(firstId)
    const secondPlayer = PlayerManager.players.get(secondId)
    ...
    const secondCompetitor = CompetitorManager.competitors.get(secondPlayer.competitorId)
    const firstEmoji = Emojis[firstCompetitor.abbreviatedName]
    const secondEmoji = Emojis[secondCompetitor.abbreviatedName]
    ...
    if (args.length === 3) {
      ...
      const firstPlayerHeroes = await PlayerManager.getHeroes(firstPlayer)
      const secondPlayerHeroes = await PlayerManager.getHeroes(secondPlayer)
      let firstIndex = -1; let secondIndex = -1

      const heroAlias = HeroManager.locateHero(args[2])
      let heroName = heroAlias
      if (heroAlias === 'wrecking-ball') {
        heroName = 'wreckingball'
      }
      const heroColor = HeroManager.getHeroColor(heroAlias)
      const heroURL = HeroManager.getHeroURL(heroAlias)
      const heroTitle = HeroManager.getHeroTitle(heroAlias)
      const heroUlt = HeroManager.getHeroUltimate(heroAlias)
      ...
      const firstInfo = []; const secondInfo = []
