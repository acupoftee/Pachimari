...
class PlayerCommand extends Command {
  ...
  async execute (client, message, args) {
    ...
    const heroes = await PlayerManager.getHeroes(player)
    ...
    if (args[1] === undefined) {
      ...
      if (player.accounts.size > 0) {
        const word = player.accounts.size > 1 ? 'Accounts' : 'Account'
        embed.addFields(`${player.accounts.size} ${word}`, `\`\`!player ${args[0]} accounts\`\``, true)
      }
      if (heroes.length > 0) {
        const word = heroes.length > 1 ? 'Heroes' : 'Hero'
      ...
      if (args[2] === undefined) {
        ...
        heroes.sort((a, b) => b.stats.time_played_total - a.stats.time_played_total).forEach(hero => {
          const info = []
          const heroMoji = Emojis[hero.name.replace('-', '').toUpperCase()]
          let heroName = HeroManager.locateHero(hero.name)
          if (heroName === 'wreckingball') {
            heroName = 'wrecking-ball'
          }
          const titleString = `${teamEmoji}${heroMoji}  ${player.givenName} '**${player.name}**' ${player.familyName}'s **${HeroManager.getHeroTitle(heroName)}** Stats`
        ...
        if (pages.length > 1) {
          ...
          const heroName = HeroManager.locateHero(heroes[0].name)
          embed.setTitle(titles[title - 1])
          embed.setDescription('')
          const heroRole = MessageUtil.capitalize(HeroManager.getHeroRole(heroName))
      ...
      let index = -1
      const heroMoji = Emojis[hero.replace('-', '').toUpperCase()]
      ...
      embed.setTitle(`${teamEmoji}${heroMoji} ${player.givenName} '**${player.name}**' ${player.familyName}'s **${HeroManager.getHeroTitle(heroes[index].name)}** Stats`)
      embed.setFooter('Stats are per 10 minutes, except for Time Played.')
      const mess = embed.buildEmbed().getEmbed
      loading.then(message => message.edit(mess))
