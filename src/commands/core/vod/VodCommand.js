...
class VodCommand extends Command {
  ...
  async execute (client, message, args) {
    ...
    const videos = []
    const pages = []; let descriptions = [] // titles = [], thumbnails = [],
    ...
    body.data.forEach(vod => {
      const voddescription = vod.description === null ? '\n\n' : `\n\n"*${vod.description}*"\n\n`
      const video = new Video(vod.unique_id, vod.available_at, vod.title, voddescription,
                `${vod.thumbnail}.jpg`, vod.share_url)
      if (vod.status === 'complete') { videos.push(video) }
    })

    for (const video of videos) {
      const date = momentTimezone(video.date).tz('America/Los_Angeles').format('ddd. MMM Do, YYYY')
      // titles.push(video.title);
      const description = `__**${video.title}**__\nPublished on ${date}${video.description}[Watch here!](${video.shareURL})\n`
