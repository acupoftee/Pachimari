...
class LiveCommand extends Command {
  ...
  async execute (client, message, args) {
    ...
    const live = body.data.liveMatch
    ...
    if (args[0] === undefined || !terms.includes(args[0].toLowerCase())) {
      ...
      if (match.state === 'IN_PROGRESS') {
        embed.setTitle(`__NOW LIVE: ${momentTimezone(match.startDateTS).tz('America/Los_Angeles').format('ddd. MMM Do, YYYY')}__`)
        ...
        embed.setTitle(`__${momentTimezone(match.startDateTS).tz('America/Los_Angeles').format('ddd. MMM Do, YYYY')}__`)
        const currentTime = new Date().getTime()
        const description = currentTime < match.startDateTS ? `*${pacificTime} / ${utcTime}*\n ${
                    Emojis[match.home.abbreviatedName.toUpperCase()]} **${match.home.name}** vs **${
                    match.away.name}** ${Emojis[match.away.abbreviatedName.toUpperCase()]}\n*Starts ${momentTimezone(match.startDateTS).endOf('minute').fromNow()}*`
          : `*${pacificTime} / ${utcTime}*\n ${Emojis[match.home.abbreviatedName.toUpperCase()]} **${match.home.name}** vs **${
                    match.away.name} ${Emojis[match.away.abbreviatedName.toUpperCase()]}**\n[Watch full match here!](https://overwatchleague.com/en-us/)`
      ...
      for (let i = 0; i < live.games.length; i++) {
        if (live.games[i].state === 'IN_PROGRESS') {
          ...
          const homeMapScore = live.games[i].points === undefined ? 0 : live.games[i].points[0]
          const awayMapScore = live.games[i].points === undefined ? 0 : live.games[i].points[1]
          const mapStr = `${Emojis[match.home.abbreviatedName.toUpperCase()]}**${match.home.name}** ||${homeMapScore} - ${
          ...
          embed.setColor(home.primaryColor)
          embed.setTitle(`__NOW LIVE: Current Map for ${momentTimezone(match.startDateTS).tz('America/Los_Angeles').format('ddd. MMM Do, YYYY')}__`)
          // embed.buildEmbed().post(message.channel);
      ...
      const pages = []; const icons = []
      let page = 1; let icon = 1
      for (let i = 0; i < live.games.length; i++) {
        const mapGuid = live.games[i].attributes.mapGuid
        const mapName = await MapManager.getMap(mapGuid)
        const mapType = await MapManager.getMapType(mapGuid)
        ...
        let mapStr
        const map = new Map(mapGuid, mapName, mapIcon, mapThumbnail, mapType, (live.games[i].state === 'PENDING'),
          live.games[i].state)
      ...
      embed.setTitle(`__Maps for Live Match: ${momentTimezone(match.startDateTS).tz('America/Los_Angeles').format('ddd. MMM Do, YYYY')}__`)
      embed.setThumbnail('')
