  ...
  async execute (client, message, args) {
    ...
    for (const player of list) {
      for (const hero of player.playedHeroes.array()) {
        const name = hero.name === 'wreckingball' ? 'wrecking-ball' : hero.name
        ...
        if (!playtimes.has(name)) {
          ...
          const currentPlaytime = playtimes.get(name)
          const updatedPlaytime = currentPlaytime + hero.timePlayed
    ...
    sortedPlaytimes.forEach(entry => {
      const heroMoji = Emojis[entry[0].replace('-', '').toUpperCase()]
      const heroRole = HeroManager.getHeroRole(entry[0])
      const roleMoji = Emojis[heroRole.toUpperCase()]
      const heroTitle = HeroManager.getHeroTitle(entry[0])
      const timePlayed = NumberUtil.toTimeString(entry[1])
      // console.log(`${heroTitle} playtime: ${timePlayed}`)
      const heroString = `\`${String(counter).padStart(2, '0')}\`. ${roleMoji}${heroMoji} **${heroTitle}:** ${timePlayed}`
