...
class Banner {
  ...
  async buildBanner (filename, first = '', second = '') {
    ...
    const homeLogo = await loadImage(this._homeLogo)
    const homeWidth = homeLogo.width / 3; const homeHeight = homeLogo.height / 3
    ...
    const awayLogo = await loadImage(this._awayLogo)
    const awayWidth = awayLogo.width / 3; const awayHeight = awayLogo.height / 3
   ...
   */
  colorIsLight (r, g, b) {
    // Counts the perceptive luminance
